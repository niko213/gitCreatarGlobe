...
namespace GlbAnalysis
{
	...
	public:
		...
		static bool CreateSection(double pts[] , int pt_num, IGlbDataset* tin_ds, std::vector<glbInt32> featureIds, const glbWChar* fieldName, IGlbGeometry** secGeo, glbWChar** secInfo);		

		static bool CreateGridSection(int x_Num, int y_Num, IGlbDataset* tin_ds, std::vector<glbInt32> featureIds, const glbWChar* fieldName,IGlbGeometry** secGeo, glbWChar** secInfo);
		...
		static bool CreateSectionByMultLine(CGlbMultiLine** multLine, IGlbDataset* tin_ds, std::vector<glbInt32> featureIds, const glbWChar* fieldName,IGlbGeometry** secGeo, glbWChar** secInfo);

		// 挖洞
		static bool CreateSection(IGlbGeometry* clipPoly, double clipDepth, IGlbDataset* tin_ds,const glbWChar* fieldName,IGlbGeometry** secGeo, glbWChar** secInfo);
	private:
		...
		//由多线段生成多边形（深度阀值以上部分）
		static bool createSectionData(std::vector<sectionData>& sectionDataVect, std::vector<sectionSeg>& sectSegVect, double depth);
		//多边形求交
		static bool intersectionPlg(sectionData& sectData, IGlbGeometry* clipPoly);
	};
}