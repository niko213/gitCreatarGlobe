...
namespace GlbGlobe
{
	...
	*/
	...
	private:
		...
		*/
		...
		public:
			...
			void updateGlobe(CGlbGlobe* globe)
			{
				mpr_globe = globe;
			}
			void run();
		...
		*/
		...
		class CGlbGlobeControlCenter  : public OpenThreads::Thread
		{
		public:
			CGlbGlobeControlCenter(CGlbGlobeView* p_view)
			{
				if( p_view == NULL)return;
				mpr_view = p_view;
				mpr_globe = p_view->GetGlobe();
				mpr_done = false;
				mpr_pause = false;
				mpr_ispaused = false;
				mpr_isrun = false;

				mpr_updatethread = p_view->mpr_updatethread;					//数据更新线程类
				mpr_renderthread =  p_view->mpr_renderthread;					//渲染更新线程类
				mpr_computeElvthread = p_view->mpr_computeElvthread;		//高程计算线程类
				mpr_dispatcherthread = p_view->mpr_dispatcherthread;			//调度线程
				//CGlbGlobeTaskManager * mpr_taskManager;
				//mpr_taskManager = NULL;
			}

			~CGlbGlobeControlCenter(void)
			{
				mpr_globe = NULL;
			}
			void run();		
		...
		private:
			...
			bool		mpr_isrun;
			CGlbGlobe* mpr_globe;
			CGlbGlobeView*  mpr_view;
			glbBool mpr_pause;
			glbBool mpr_ispaused;

			CGlbGlobeView::CGlbGlobeUpdateThread* mpr_updatethread;					//数据更新线程类
			CGlbGlobeView::CGlbGlobeRenderThread* mpr_renderthread;					//渲染更新线程类
			CGlbGlobeView::CGlbGlobeComputeElevationThread* mpr_computeElvthread;		//高程计算线程类
			CGlbGlobeView::CGlbGlobeDispatcherThread* mpr_dispatcherthread;			//调度线程
			//OpenThreads::Thread * mpr_featureLayerThread;
			//CGlbGlobeTaskManager * mpr_taskManager;
		};
	...
	public:	
		...
		*		  [out] InterPoints 选中对象的交点集合[世界坐标]
		* @return 有对象选中返回 true
		*		   没有对象选中返回false
		*/
		...
		*		   没有对象选中返回false
		*/
		glbBool PickTiltPhotographyLayer( glbInt32 mousex,glbInt32 mousey, std::vector<glbref_ptr<CGlbGlobeLayer>>&results,std::vector<osg::Vec3d>&InterPoints,std::vector<glbInt32>&featureOids);
		/**
		...
		*		-	失败返回false
		**/
		glbBool EnableRain(glbBool bEnable);
		glbBool IsRain();
		/**
		* @brief 启动/关闭 雪效
		* @return 成功返回true
		*		-	失败返回false
		**/
		glbBool EnableSnow(glbBool bEnable);
		glbBool IsSnow();
		/**
		* @brief 启动/关闭 雾效
		* @return 成功返回true
		*		-	失败返回false
		**/
		glbBool EnableFog(glbBool bEnable);
		glbBool isFog();
		...
		*		 [in] labelColor 文字颜色
		**/
		bool SetLogoLabel(const glbWChar* label, glbInt32 labelColor);
		/**
		* @brief 设置logo图片
		* @param [in] path 图片路径
		**/
		bool SetLogoImage(const glbWChar* path);
	//public:
	//	// 释放编译context生成的资源
	//	void FreeCompileContextResource();
	private:
...
#ifdef _GlbNETLOCK 
		bool glbVertifyNetDog(std::string& lastError);  
		void globeCloseNetDog();
#endif
	public:
	...
	private:
		...
		CGlbGlobeControlCenter* mpr_controlCenter; //线程控制中心
		...
		glbBool												mpr_isSnow;
		glbBool												mpr_isFog;
		osg::ref_ptr<osgParticle::PrecipitationEffect>		mpr_pPEffect;
	};
}