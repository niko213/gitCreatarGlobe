...
namespace GlbGlobe
{
	...
	* @brief 图例类
	*/
	class GLB_DLLCLASS_EXPORT CGlbLegend : public CGlbReference
	{
	public:
		CGlbLegend(glbInt32 vx,glbInt32 vy, glbInt32 vWidth,glbInt32 vHeight, CGlbColorRamp* colorramp);
		~CGlbLegend();
	...
	protected:
		/**
		* @brief 图例视口更新回调类（3D窗口改变大小时，图例位置自动更新）
		*/
		class ResetViewportCallback : public osgGA::GUIEventHandler
		{
		public:
			ResetViewportCallback(CGlbLegend* lgd)
			{
				m_lgd = lgd;
			}
			virtual bool handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa, osg::Object* obj, osg::NodeVisitor*)
			{
				osgGA::GUIEventHandler::handle(ea, aa);
	...
	private:
		...
		osg::ref_ptr<LegendDrawable> legenddraw;//文字绘制节点
